...
const rockPlaylist = []; // Tenemos el array vacío

for (let i = 0; i < songs.length; i++) { // Usamos un bucle for para recorrer el array
  if (songs[i].genre === 'Rock' && songs[i].duration > 5) {
  //Aquí filtramos las canciones que sean de género Rock y que duren más de 5 minutos
    rockPlaylist.push(songs[i]); // Si se cuemple, agregamos la canción al array rockPlaylist
  }
}
console.log('Playlist de canciones de Rock que duran más de 5 minutos:');
